//! STC scanner: checks Started Task definitions for security issues.
...
/// Run STC scanner with a given configuration.
pub fn scan_stc_with_config(config: &Config) -> Result<Vec<Findings>, MsasError> {
    internal_scan_stc(Some(config))
}

/// Run STC scanner using default configuration.
pub fn scan_stc() -> Result<Vec<Findings>, MsasError> {
    internal_scan_stc(None)
}

fn internal_scan_stc(config_opt: Option<&Config>) -> Result<Vec<Findings>, MsasError> {
    ...
    let local_output = {
        ...
        format!("{}/stc_{}.txt", dir.display(), std::process::id())
    };
...
pub fn parse_output(contents: &str) -> Result<Vec<Findings>, MsasError> {
    ...
    for line in contents.lines() {
        ...
        findings.push(Findings {
            id: "STC-WEAK-CONFIG".to_string(),
            title: trimmed.to_string(),
            severity,
            affected_resource: affected,
            remediation: "Review STARTED task definitions and associated user privileges"
                .to_string(),
            compliance: None,
        });
    }
