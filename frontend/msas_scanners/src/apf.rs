//! APF scanner checks for weak permissions on APF‑authorized datasets.
...
/// Run APF scanner with a given configuration.
pub fn scan_apf_with_config(config: &Config) -> Result<Vec<Findings>, MsasError> {
    internal_scan_apf(Some(config))
}

/// Run APF scanner using default configuration.
pub fn scan_apf() -> Result<Vec<Findings>, MsasError> {
    internal_scan_apf(None)
}

fn internal_scan_apf(config_opt: Option<&Config>) -> Result<Vec<Findings>, MsasError> {
    ...
    let local_output = {
        ...
        format!("{}/apf_{}.txt", dir.display(), std::process::id())
    };
...
pub fn parse_output(contents: &str) -> Result<Vec<Findings>, MsasError> {
    ...
    for line in contents.lines() {
        ...
        findings.push(Findings {
            id: "APF-WEAK-PERMISSION".to_string(),
            title: trimmed.to_string(),
            severity,
            affected_resource: affected,
            remediation: "Restrict access to APF‑authorized datasets and review APF list"
                .to_string(),
            compliance: None,
        });
    }
