//! Unix scanner: checks OMVS (USS) for security misconfigurations.
...
/// Run Unix scanner with a given configuration.
pub fn scan_unix_with_config(config: &Config) -> Result<Vec<Findings>, MsasError> {
    internal_scan_unix(Some(config))
}

/// Run Unix scanner using default configuration.
pub fn scan_unix() -> Result<Vec<Findings>, MsasError> {
    internal_scan_unix(None)
}

fn internal_scan_unix(config_opt: Option<&Config>) -> Result<Vec<Findings>, MsasError> {
    ...
    let local_output = {
        ...
        format!("{}/unix_{}.txt", dir.display(), std::process::id())
    };
...
pub fn parse_output(contents: &str) -> Result<Vec<Findings>, MsasError> {
    ...
    for line in contents.lines() {
        ...
        findings.push(Findings {
            id: "UNIX-WEAK-PERMISSION".to_string(),
            title: trimmed.to_string(),
            severity,
            affected_resource: affected,
            remediation: "Review Unix file permissions and SUID binaries".to_string(),
            compliance: None,
        });
    }
